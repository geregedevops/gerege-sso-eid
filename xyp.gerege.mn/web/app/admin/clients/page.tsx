import { listClients, deactivateClient } from "@/lib/api";
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white">Verify Clients</h1>
          <p className="text-sm text-slate-400 mt-1">
            Бүртгэлтэй API client-ийн жагсаалт
          </p>
        </div>
        <a
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/[0.06]">
              ...
              <th className="text-left px-5 py-3 text-slate-400 font-semibold">Имэйл</th>
              <th className="text-left px-5 py-3 text-slate-400 font-semibold">Rate Limit</th>
            ...
            {clients.length === 0 ? (
              ...
              clients.map((c: any) => (
                <tr key={c.id} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                  ...
                  <td className="px-5 py-3 text-slate-400 text-xs">{c.contact_email || "-"}</td>
                  <td className="px-5 py-3 text-slate-400 text-xs">{c.rate_limit}/min</td>
